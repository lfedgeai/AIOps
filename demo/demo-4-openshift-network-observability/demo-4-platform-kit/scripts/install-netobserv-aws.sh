...
LOKI_INGESTION_RATE="${LOKI_INGESTION_RATE:-10}"          # MB/s — raised for full-demo NetObserv flow volume
LOKI_INGESTION_BURST_SIZE="${LOKI_INGESTION_BURST_SIZE:-20}"  # MB per distributor push burst
...
if [[ -n "${AWS_ACCESS_KEY_ID:-}" && -n "${AWS_SECRET_ACCESS_KEY:-}" \
  && -n "${AWS_REGION:-}" && -n "${S3_BUCKET:-}" ]]; then
  ok "Using AWS configuration from environment (site-config / greenfield)."
...
step "NetObserv operator Secret RBAC (console plugin + Loki integration)"
INSTALL_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
chmod +x "$INSTALL_ROOT/scripts/fix-netobserv-operator-rbac.sh" 2>/dev/null || true
if LOKI_NS="$LOKI_NS" NETOBSERV_NS="$NETOBSERV_NS" OPERATOR_NS="$OPERATOR_NS" \
  "$INSTALL_ROOT/scripts/fix-netobserv-operator-rbac.sh" apply; then
  ok "Operator Secret rolebindings applied."
else
  warn "Could not apply operator Secret RBAC yet — re-run: ./scripts/fix-netobserv-operator-rbac.sh"
fi
...
${c_green}=============================================================${c_reset}
 Network Observability install complete.
   Loki namespace   : ${LOKI_NS}  (size ${LOKI_SIZE}, ingest ${LOKI_INGESTION_RATE}/${LOKI_INGESTION_BURST_SIZE} MB/s/burst, class ${STORAGE_CLASS})
 ...
 If distributor 429 during demo:  ./scripts/tune-loki-ingestion.sh
 If FlowCollector WebConsoleError: ./scripts/fix-netobserv-operator-rbac.sh apply
 After cluster restart:            ./scripts/post-cluster-resume.sh
${c_green}=============================================================${c_reset}
SUMMARY
