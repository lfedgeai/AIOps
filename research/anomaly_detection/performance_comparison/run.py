...
GROUND = ROOT / "ground_truth_data"
DATA = ROOT / "otel-demo" / "otel_ground_truth_data"
...
def collect_baselines(windows: int, cooldown: int, duration: int, keep: bool) -> None:
    """
    Non-deleting baseline collector using the existing ground_truth_data_collector utilities.
    """
...
def run_suite() -> None:
    # Use the existing full suite collector (fault injections + baselines)
    _run([sys.executable, str(GROUND / "collect_full_suite.py")])


def build_features() -> None:
    # Build features and write features.csv
    env = os.environ.copy()
...
def train_eval() -> None:
    # feature_pipeline.py already trains and evaluates
    build_features()
...
def _resolve_dataset_path(dataset: str | None) -> Path:
    ...
    if dataset:
        ...
        if p.exists():
            return p
        # Treat dataset as a named subfolder under datasets/
...
def main() -> int:
    p = argparse.ArgumentParser(description="AIOps Harness CLI")
